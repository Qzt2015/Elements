...
{
    [Serializable]
    public class Transformer_3 : Element //三绕阻变压器
    {
        public Transformer_3(float v1, float v2, float v3, float v4, int v5)
        {
            X = v1;
            Y = v2;
            W = v3;
        ...
        public override void Draw(Graphics g, Pen mypen,SolidBrush brush)
        {
            ...
            g.DrawString(name + "  电压:" + vbase, new Font("微软雅黑", 7 * (float)(Math.Ceiling(double.Parse(B.ToString())))), Brushes.Red, new RectangleF(r8.X + W / 4 * B, r8.Y, W * (B + 1), H * (B + 1)));
            //g.DrawRectangle(mypen, Rectangle.Round(rect));
        }
    }
}
